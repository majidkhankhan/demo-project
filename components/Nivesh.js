    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "https://api.sadaivsatya.com/api/home/nivesh"
        );
        ...
        >
          ...
          <div className="ak-block-inner clearfix">
            <div className="ak-block-posts clearfix">
              {postData.nivesh.slice(0, 1).map((post) => (
                <article
                  ...
                  <div className="ak-module-inner clearfix">
                    <div className="ak-module-grid-wrap">
                      <div className="ak-module-featured">
                        ...
                        <div className="ak-featured-cover">
                          <Link
                            ...
                            rel="bookmark"
                            aria-label={stripHtmlTags(post.newsHeading, 50)}
                          ...
                          >
                            <div className="ak-featured-thumb lazy-thumb size-715">
                              <img
                                width={350}
                                height={250}
                                src={`${image_resize}?url=${BASE_URL}${post.newsImage}&w=319&h=228&outtype=webp`}
                                className="attachment-newsy_350x250 size-newsy_350x250 wp-post-image lazyautosizes ls-is-cached lazyloaded"
                                alt=""
                                decoding="async"
                                loading="lazy"
                                data-src={`${image_resize}?url=${BASE_URL}${post.newsImage}&w=319&h=228&outtype=webp`}
                                data-sizes="auto"
                                data-srcset={`${image_resize}?url=${BASE_URL}${post.newsImage}&w=319&h=228&outtype=webp`}
                                data-expand={700}
                                sizes="335px"
                                srcSet={`${image_resize}?url=${BASE_URL}${post.newsImage}&w=319&h=228&outtype=webp`}
                              />
                            </div>
                          </Link>
                        </div>
                      ...
                      <div className="ak-module-details">
                        <h1 className="ak-module-title mb-1">
                          <Link
                            href={`/BusinessDetailsD/${post.mainKhabarId}`}
                            rel="bookmark"
                            aria-label={stripHtmlTags(post.newsHeading, 50)}
              ...
              {postData.nivesh.slice(1, 5).map((post) => {
