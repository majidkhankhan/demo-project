...
function Television() {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "https://api.sadaivsatya.com/api/home/lifestyle"
        );
  ...
  const handleNewsClick = (trendingTagId) => {
    router.push(`/NewsDetails/${trendingTagId}`);
  };
        ...
        >
          ...
          <div className="ak-block-inner clearfix">
            <div className="ak-block-posts clearfix">
              {postData.lifestyle.slice(0, 1).map((post) => (
                <article
                          ...
                          >
                            <div className="ak-featured-thumb lazy-thumb size-715">
                              <img
                                width={350}
                                height={250}
                                src={`${image_resize}?url=${BASE_URL}${post.newsImage}&w=313&h=224&outtype=webp`}
                                className="attachment-newsy_350x250 size-newsy_350x250 wp-post-image lazyautosizes ls-is-cached lazyloaded"
                                alt=""
                                decoding="async"
                                loading="lazy"
                                data-src={`${image_resize}?url=${BASE_URL}${post.newsImage}&w=313&h=224&outtype=webp`}
                                data-sizes="auto"
                                data-srcset={`${image_resize}?url=${BASE_URL}${post.newsImage}&w=313&h=224&outtype=webp`}
                                data-expand={700}
                                sizes="335px"
                                srcSet={`${image_resize}?url=${BASE_URL}${post.newsImage}&w=313&h=224&outtype=webp`}
                              />
                            </div>
                          </Link>
                        </div>
              ...
              {postData.lifestyle.slice(1, 5).map((post) => {
                          ...
                          >
                            <div className="ak-featured-thumb lazy-thumb size-715">
                              <img
                                width={120}
                                height={90}
                                src={`${image_resize}?url=${BASE_URL}${post.newsImage}&w=120&h=90&outtype=webp`}
                                className="attachment-newsy_120x86 size-newsy_120x86 wp-post-image lazyautosizes ls-is-cached lazyloaded"
                                alt=""
                                decoding="async"
                                loading="lazy"
                                data-src={`${image_resize}?url=${BASE_URL}${post.newsImage}&w=120&h=90&outtype=webp`}
                                data-sizes="auto"
                                data-srcset={`${image_resize}?url=${BASE_URL}${post.newsImage}&w=120&h=90&outtype=webp`}
                                data-expand={700}
                                sizes="120px"
                                srcSet={`${image_resize}?url=${BASE_URL}${post.newsImage}&w=120&h=90&outtype=webp`}
                              />
                            </div>
                          </Link>
                        </div>
